...
| **Текст** | поля двух заголовков с таймингами и слова выбранного файла: текст, начало, конец, удалить, вставить |
| **Настройки стиля** | шрифт, регистр, разбивка текста, цвета, обводка, тени, подсветка, позиция, анимация, стили обоих заголовков |
...
- **Позиция** — верх, центр или низ, отступ от края, число строк и ширина
  блока.
- **Межстрочный интервал** — расстояние между строками в долях высоты строки
  (1.18 по умолчанию). Работает на тексте в две строки и больше.
...
Поверх видео можно положить два текстовых блока — например, рубрику и
подзаголовок. Оба видны одновременно и живут каждый своё время.

<img src="docs/result-frame.png" alt="Кадр готового ролика: два заголовка разного стиля сверху и субтитр по одному слову ниже" width="300" align="right">

- **Текст и тайминги** — в колонке «Текст», в блоке «ЗАГОЛОВКИ»: два поля и
  по два числа, начало и конец в секундах. Двойной клик по полю времени берёт
  текущий момент с плеера. Пустой заголовок просто не рисуется.
- **Стиль** — в правой панели, разделы «Заголовок 1» и «Заголовок 2». У
  каждого свои шрифт, размер, цвет, обводка, две тени, плашка или цветная
  подсветка, позиция, отступ от края и ширина блока.
- Регистр, разбивка и число строк заголовку не нужны: текст переносится и
  уменьшается сам, чтобы поместиться в кадр. Межстрочный интервал у каждого
  заголовка свой.
- **Сдвиг влево / вправо** — положение заголовка по горизонтали в пикселях
  исходника: 0 — по центру кадра, минус влево, плюс вправо. У каждого
  заголовка свой, так что два заголовка можно развести по разным углам. За
  край кадра заголовок не уедет: сдвиг упирается в границу.
- **Анимация** — отдельно на появление (Fade in) и на исчезновение
  (Fade out). У каждой свой выключатель, вид и длительность в миллисекундах:
  плавно, выезд с любой из четырёх сторон, приближение или отдаление,
  растворение через размытие.
- Появление и исчезновение никогда не занимают больше половины времени
  заголовка, поэтому короткий заголовок не превращается в одну сплошную
  анимацию.
...
При запуске программа читает файл `version.json` из репозитория на GitHub и
сравнивает номер со своим. Если вышла новая версия, кнопка **Check updates** в
шапке превращается в **Доступна 1.3.0** и по нажатию открывает репозиторий; в
подсказке к кнопке — что в этой версии изменилось. Кнопку можно нажать и
самому в любой момент. Проверка ничего не скачивает и не отправляет: один
запрос к `raw.githubusercontent.com` за файлом в двести байт. Нет интернета —
кнопка молча остаётся как была.

Само обновление — вручную. Если программа установлена через `git clone`:
